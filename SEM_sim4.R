...
sampdata_boot=function(d,nsamp_sub,samp_prop,nbootsamp=1000,bootthresh=25) {
  ...
  if (samp_prop<1){
    for (j in 1:nsamp_sub) {
      ...
      idx=sort(jhist[,3],index.return=TRUE)$ix
      jhist=jhist[idx,]
      if (min(jhist[,3])>=bootthresh) {
        bsamp_cov=c()
        for (i in 1:nbootsamp) {
          dsim_bsamp=dsim_samp[sample.int(j-1,j-1,replace=TRUE),]
          tmpcov=cov(dsim_bsamp,use='pairwise.complete')
          bsamp_cov=rbind(bsamp_cov,tmpcov[upper.tri(tmpcov)])
        }
        bsamp_cov_var=matrix(0,ncol=ncol(tmpcov),nrow=nrow(tmpcov))
        bsamp_cov_var[upper.tri(tmpcov)]=apply(bsamp_cov,2,var)
        jvar=c()
        for (ii in 1:nrow(tmpcov)) {
          for (jj in ii:nrow(tmpcov)) {
            if (ii!=jj) {
              jvar=rbind(jvar,c(ii,jj,bsamp_cov_var[ii,jj]))
            }
          }
        }
        idx=sort(jvar[,3],decreasing=TRUE,index.return=TRUE)$ix
        jhist=jvar[idx,]
        
        print(sprintf('%d: using bootstrap (%f- %f)',j,min(jhist[,3]),max(jhist[,3])))
...
dsim_opt=sampdata_boot(data2,nrow(dsim_forms),2/9)
...
if (imp==1) {outdir='outputs_amelia'} else {outdir='outputs_fiml_opt'}
